...
dataset_mod <- subset(x = dataset_mod, select = -c(latitude, longitude))
#View(dataset_mod)
```
...
smp_size <- floor(0.75 * nrow(dataset_mod))

## set the seed to make your partition reproducible
set.seed(123)
train_ind <- sample(seq_len(nrow(dataset_mod)), size = smp_size)

train_data <- dataset_mod[train_ind, ]
test_data <- dataset_mod[-train_ind, ]
```
...
## transform data to lists, so that keras can work with it
```{r}
