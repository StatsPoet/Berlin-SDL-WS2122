...
```{r}
image_an_mvars_df <- image_an_mvars_df[,-c(which(colnames(image_an_mvars_df) == "Alarm_clock_pic_10"):which(colnames(image_an_mvars_df) == "Washing_machine_pic_17"))]
```

```{r}
image_an_mvars_df <- image_an_mvars_df[,-c(which(colnames(image_an_mvars_df) == "brightness_pic_10"):which(colnames(image_an_mvars_df) == "brightness_pic_17"))]
```
...
Remove all dummy variables with "Bathroom_cabinet_pic_0" being the first dummy and "Washing_machine_pic_9" the last. 
```{r}
image_an_mvars_df_sums_only  <- image_an_mvars_df[,-c(which(colnames(image_an_mvars_df) == "Bathroom_cabinet_pic_0"):which(colnames(image_an_mvars_df) == "Washing_machine_pic_9"))]
```
...
```{r}
any(image_an_mvars_df_ind_pic_only[,"brightness_mean"] == 0)
all(is.numeric(image_an_mvars_df_ind_pic_only[,"brightness_mean"]))
```
